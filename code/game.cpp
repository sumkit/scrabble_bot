...
class Game {
	...
	/*bool adjacentAbove(int row, int col) {
		...
		if(sp.spaceTile != NULL) {
			if(sp.row == 0) return false;
			if(board[(row-1)*DIMENSION + col].spaceTile == 0) return true;
		}
		return false;
	}
	...
	bool adjacentLeft(int row, int col) {
		...
		if(sp.spaceTile != NULL) {
			...
			if(board[row*DIMENSION + col - 1].spaceTile == 0) return true;
		}
		return false;
	}
	...
	bool Game::validWord(std::string str) {
		//check in trie
		int index = 1;
